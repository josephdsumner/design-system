...
var anchoredPositionModifier = modifier((element, positional, named = {}) => {
  ...
  if (named?.arrowOptions?.element) {
    assert('`hds-anchored-position` modifier - the `element` provided for the "arrow" element is not a valid DOM node', named.arrowOptions.element instanceof HTMLElement || named.arrowOptions.element instanceof SVGElement);
    arrowElement = named.arrowOptions.element;
  } else if (named?.arrowOptions?.selector) {
    assert('`hds-anchored-position` modifier - the `selector` provided for the "arrow" element must be a string', typeof named.arrowOptions.selector === 'string');
